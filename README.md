...
## Fork: OneDrive Auth Fixes + Ubuntu/Android Install Scripts

This branch (`fix/onedrive-auth-hang`) fixes the OneDrive auth flow hanging forever on "Connecting to OneDrive... Please DO NOT close this modal." Upstream context: issues [#1185](https://github.com/remotely-save/remotely-save/issues/1185), [#1170](https://github.com/remotely-save/remotely-save/issues/1170) and PR [#1186](https://github.com/remotely-save/remotely-save/pull/1186).

What was wrong and what changed:

1. **First-connect 400 (the hang).** On a fresh vault the plugin created the remote folder via `POST /drive/special/approot/children`, which Graph rejects with `400 invalidRequest`. It now uses idempotent `PATCH` on the item path (same approach as `_mkdirFromRoot`), for both App Folder (`src/fsOnedrive.ts`) and Full (`pro/src/fsOnedriveFull.ts`).
2. **Silent hangs.** The Dropbox/OneDrive/OneDriveFull protocol handlers now surface failures inside the modal (instead of hanging), reset the PKCE verifier, and `sendAuthReq` rethrows so callers never get `undefined`. Also added the missing `await` on `setConfigBySuccessfullAuthInplace*`.
3. **Instantly-expired tokens.** `accessTokenExpiresAtTime` added `expires_in` seconds as milliseconds; now `* 1000` like the refresh path.
4. **Graph `/me` outages.** The profile endpoint intermittently returns `500`/`504` while Drive is healthy. The display name now falls back to owner info from `GET /drive` (`extractDisplayNameFromDriveInfo` in `src/misc.ts`), and Check proves connectivity via `_init()` when the profile service is down.
5. **Build fixes.** Added missing `terser-webpack-plugin`, `transpileOnly` for ts-loader (pre-existing `SharedArrayBuffer` tsc errors), `aggregate-error` 5→3 (v5 pulls ESM `clean-stack` with `node:` imports webpack can't bundle).
...
```bash
./install-android.sh --vault-device-path /sdcard/Obsidian/MyVault  # via adb
./install-android.sh --manual                                       # USB copy steps
```

Then on the phone: Obsidian → Settings → Community plugins → enable Remotely Save → OneDrive → Auth. Keep the vault name identical to desktop so both sync the same remote folder.
